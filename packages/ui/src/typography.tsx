...
export function Lead({ className, ...props }: HTMLMotionProps<'p'>) {
	return <motion.p variants={itemVariants} className={cn('font-body text-muted-foreground text-lg leading-relaxed md:text-xl', className)} {...props} />
}
...
export function Muted({ className, ...props }: HTMLMotionProps<'p'>) {
	return <motion.p variants={itemVariants} className={cn('text-muted-foreground text-sm', className)} {...props} />
}
...
export function InlineCode({ className, ...props }: React.HTMLAttributes<HTMLElement>) {
	return <code className={cn('bg-muted relative rounded px-[0.3rem] py-[0.2rem] font-mono text-sm font-semibold', className)} {...props} />
}
